...
async def next_page(bot, query):
    ...
    btn.insert(0, [
        InlineKeyboardButton("! Get All !", callback_data=f"send_fall#files#{offset}#{req}"),
        InlineKeyboardButton("! Lᴀɴɢᴜᴀɢᴇs !", callback_data=f"select_lang#{req}")
    ])
    btn.insert(0, [
        InlineKeyboardButton("⚡Join Mr Movie Premium⚡", url=f"https://t.me/{temp.U_NAME}")
    ])
    try:
...
async def language_check(bot, query):
    ...
    if files:
        ...
        btn.insert(0, [
            InlineKeyboardButton("! Get All !", callback_data=f"send_fall#{pre}#{0}#{userid}"),
...
async def select_language(bot, query):
    ...
    btn = [[
        ...
        InlineKeyboardButton("Bangla", callback_data=f"lang#{userid}#bn"),
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                if clicked == typed:
                    ...
                    await query.answer(f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. it's others!", show_alert=True)
            elif settings['botpm']:
                if clicked == typed:
                    await query.answer(url=f"https://t.me/{temp.U_NAME}?start={ident}_{file_id}")
                    return
                ...
                if clicked == typed:
                    ...
                    return await query.answer(f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. it's others!", show_alert=True)
        ...
        if file_id == "send_all":
            ...
            if is_over == 'done':
                ...
                return await query.answer("Hᴇʏ, Yᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴊᴏɪɴᴇᴅ ɪɴ Our ʙᴀᴄᴋ ᴜᴘ ᴄʜᴀɴɴᴇʟ. Cʜᴇᴄᴋ ᴍʏ PM ᴛᴏ ᴊᴏɪɴ ᴀɴᴅ ɢᴇᴛ ғɪʟᴇs !", show_alert=True)
            elif is_over == 'verify':
                return await query.answer("Hᴇʏ, Yᴏᴜ ʜᴀᴠᴇ ɴᴏᴛ ᴠᴇʀɪғɪᴇᴅ ᴛᴏᴅᴀʏ. Yᴏᴜ ʜᴀᴠᴇ ᴛᴏ ᴠᴇʀɪғʏ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ. Cʜᴇᴄᴋ ᴍʏ PM ᴛᴏ ᴠᴇʀɪғʏ ᴀɴᴅ ɢᴇᴛ ғɪʟᴇs !", show_alert=True)
            else:
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0, [
        InlineKeyboardButton("! Get All !", callback_data=f"send_fall#{pre}#{0}#{message.from_user.id}"),
